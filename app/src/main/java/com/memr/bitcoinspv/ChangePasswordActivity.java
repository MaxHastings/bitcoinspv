...
public class ChangePasswordActivity extends WalletAppKitActivity {
    ...
    protected void onWalletKitReady() {
        setContentView(com.memr.bitcoinspv.R.layout.activity_change_password);
        passwordManager = ((BitcoinSPV) getApplication()).getPasswordManager();

        currentPasswordLayout = findViewById(com.memr.bitcoinspv.R.id.currentPasswordInputLayout);
        newPasswordLayout = findViewById(com.memr.bitcoinspv.R.id.newPasswordLayout);
        reNewPasswordLayout = findViewById(com.memr.bitcoinspv.R.id.reNewPasswordLayout);
        changePasswordButton = findViewById(com.memr.bitcoinspv.R.id.changePasswordButton);
        encryptWalletBox = findViewById(com.memr.bitcoinspv.R.id.encryptWallet);

        progressBar = findViewById(com.memr.bitcoinspv.R.id.progressBar);
        progressBar.setVisibility(View.GONE);

        noPasswordBox = findViewById(com.memr.bitcoinspv.R.id.noPasswordBox);
    ...
    private boolean isInputValid(){
        if(doesPasswordExist()){
            if(isCurrentPasswordCorrect(getCurrentPassword())){
                ...
                currentPasswordLayout.setError(getString(com.memr.bitcoinspv.R.string.password_is_incorrect));
                return false;
            }
        }else{
            return true;
            ...
            public void run() {
                try {
                    ...
                    runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            Toast.makeText(ChangePasswordActivity.this, getString(com.memr.bitcoinspv.R.string.password_updated), Toast.LENGTH_LONG).show();
                            finish();
                        }
                    });
    ...
    private void failed(String error){
        ...
        progressBar.setVisibility(View.GONE);
        changePasswordButton.setText(com.memr.bitcoinspv.R.string.update_password_button);
